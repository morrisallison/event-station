...
class EventStation {
    ...
    toObservable(q, s, selector) {
        ...
        return deps.$RxObservable.fromEventPattern((r) => {
            this.on(q, r, s);
        }, (r) => {
            this.off(q, r, s);
        }, selector);
...
function emitEvent(eventName, originStation, enableAsync, args) {
    ...
    var promises = [];
    if (listeners) {
        let result = applyListeners(listeners, originStation, enableAsync, args);
    ...
    if (stationMeta.emitAllEvent && listenersMapAll) {
        ...
        let result = applyListeners(listenersMapAll, originStation, enableAsync, argsAll);
        if (enableAsync && result) {
            promises = promises.concat(result);
        }
    }
