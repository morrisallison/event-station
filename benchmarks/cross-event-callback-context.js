...
var listeners = station.hear(station3, 'foobar', callback, context);
listeners.detach();

console.log("Event, callback, context, and cross-emitter listening");

suite

    .add('Event-Station', function () {
        station.hear(station2, 'foobar', callback, context);
        station2.emit('foobar');
        station.disregard(station2, 'foobar', callback, context);
    })

    .add('Event-Station (2)', function () {
        var listeners = station.hear(station2, 'foobar', callback, context);
        station2.emit('foobar');
    ...
    .add('Backbone.Events', function () {
        backbone.listenTo(backbone2, 'foobar', callback, context);
        backbone2.trigger('foobar');
        backbone.stopListening(backbone2, 'foobar', callback, context);
    })
